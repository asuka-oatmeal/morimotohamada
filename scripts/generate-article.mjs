...
- 最後に「まとめ」セクションを入れる
- マークダウン形式で出力する
- 記事中に必ず1〜2個のMermaidダイアグラム（フローチャートや図解）を含める

【Mermaidダイアグラムのルール】
- \`\`\`mermaid で囲んで記述する
- ダイアグラムの直前に「<!-- alt: ここに図の説明を書く -->」の形式でalt属性用の説明を必ず入れる
- フローチャート（graph TD）や比較図を使い、手続きの流れや要件の整理を視覚化する
- ノードのテキストは日本語で書く
- ノードIDには日本語を使わず英数字のみにする（例: A[申立て] ○、申立て[申立て] ×）

【Mermaidの記述例】
<!-- alt: 離婚調停の手続きの流れを示すフローチャート -->
\`\`\`mermaid
graph TD
    A[家庭裁判所に申立て] --> B[第1回調停期日]
    B --> C{合意成立?}
    C -->|はい| D[調停成立]
    C -->|いいえ| E[調停不成立]
    E --> F[裁判へ]
